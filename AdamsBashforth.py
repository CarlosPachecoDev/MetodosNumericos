...
def adams_bashforth(ecuacion, paso, valor_y_inicial, valor_x_inicial, valor_x_final,h):
    print("BASHFORT")
    ...
    d, yi = runge_kutta(ecuacion, valor_y_inicial=valor_y_inicial, valor_x_inicial=valor_x_inicial,orden=paso,h=h,valor_x_final=valor_x_final)
    yi = yi[:int(paso[0])-1]
...
def solve_multipasos():
    ...
    main_layout = [
        ...
        [sg.Text('Pasos: ', key="-TXT|ORDEN-"), sg.Image(radio_unchecked, enable_events=True, k='2', metadata=False), sg.T('2', k="T2"), sg.Image(radio_unchecked, enable_events=True, k='4', metadata=False), sg.T('4', k="T4")],
    ...
    while True:
        ...
        if event == "-BTN|SOLVE-":
            ...
            if values["-IN|H-"] != None:
                respuesta = adams_bashforth(ecuacion=values['-IN|FX-'], valor_y_inicial=float(values['-IN|VALY-']), valor_x_inicial= float(values['-IN|VALX-']), h=float(values['-IN|H-']), paso=paso, valor_x_final=float(values['-IN|VALXI-']))
            else:
