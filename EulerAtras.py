...
def euler(valor_y_inicial, ecuacion, valor_x_inicial, valor_x_final, n=None, h = None):
    ...
    while valor_x_inicial < valor_x_final:
        i += 1
        y_n_1 = valor_y_inicial + h*ecuacion.subs([(y, valor_y_inicial), (x, valor_x_inicial)]).evalf()
        valor_x_inicial = round(valor_x_inicial + h,2)
        y_n = valor_y_inicial + h*ecuacion.subs([(y, y_n_1), (x, valor_x_inicial)]).evalf()
        valor_y_inicial = y_n
        salida.append([i,valor_x_inicial,valor_y_inicial])
    
    return salida

def solve_euler_atras():
    ...
    atras_euler_window = sg.Window('', main_layout, no_titlebar=True, margins=(0,0))

    while True:
        event, values = atras_euler_window.read()
        if event == "-BTN|FX-":
            atras_euler_window["-IN|FX-"].update(showCalculator())
        ...
        if event == "-BTN|SOLVE-":
            ...
            atras_euler_window['-TXT|RES-'].update(values=respuesta)

        if event == "Salir" or event == sg.WIN_CLOSED:
            break
    
    atras_euler_window.close()
