...
def euler(valor_y_inicial, ecuacion,valor_x_inicial, valor_x_final, n=None, h = None):
    ...
    while valor_x_inicial < valor_x_final:
        i += 1
        y_n1 = valor_y_inicial + h*ecuacion.subs([(y, valor_y_inicial), (x, valor_x_inicial)])
...
def solve_euler_centrado():
    ...
    centrado_euler_window = sg.Window('', main_layout, no_titlebar=True, margins=(0,0))

    while True:
        event, values = centrado_euler_window.read()
        if event == "-BTN|FX-":
            centrado_euler_window["-IN|FX-"].update(showCalculator())
        ...
        if event == "-BTN|SOLVE-":
            ...
            centrado_euler_window['-TXT|RES-'].update(values=respuesta)

        if event == "Salir" or event == sg.WIN_CLOSED:
            break
    
    centrado_euler_window.close()
