...
from DiffDivididas import diferencias

def newton(expresion= None, datos= None, valor_eval= None, intervalo= None, grado_polinomio = None):
    ...
    if expresion != None:
        ...
        if intervalo != None:
            ...
            for i in range(1,n):
                ...
                termino = 1
                for k in range(i):
                    termino = termino*(variables[0]-xi[k])
                print(f"Termino:{termino} \tFactor:{factor}")
            ...
            if valor_eval != None:
                ...
                valor_verdadero = expresion.subs(variables[0],valor_eval).evalf()
                resultado_evaluacion = polisimple.subs(variables[0],valor_eval).evalf()
                ...
                n = len(xi)-1
                numerador = sp.diff(expresion,variables[0],n+1)
        ...
        #Si el usuario proporcionó una tabla de valores de x
        else:
            ...
            for i in range(n):
                fi.append(expresion.subs(variables[0],xi[i]).evalf())
    ...
    #Significa que el usuario proporcionó una tabla de valores
    else:
        xi = [float(number) for number in datos[0]]
        ...
        if valor_eval != None:
            ...
            error_teorico = f"\nEl error teórico es: {math.fabs(((numerador.subs(x,valor_eval).evalf())/(math.factorial(n+1)))*segunda_expresion)}"
...
def solve_newton():
    ...
    newton_window = sg.Window('', main_layout)
    ...
    def check_radio(key,buttomgroup):
        for k in buttomgroup:
            newton_window[k].update(radio_unchecked)
            newton_window[k].metadata = False
        newton_window[key].update(radio_checked)
        newton_window[key].metadata = True
    ...
    while True:
        ...
        event, values = newton_window.read()
        print(event)
        ...
        if event in INT_INPUTS and len(values[event]) and values[event][-1] not in ('1234567890'):
            newton_window[event].update(values[event][:-1])
        ...
        if event == "-COMBO|DATOS-":
            ...
            if values["-COMBO|DATOS-"] == "Función Matemática":
                Buttom_flag = False
                reset(["-IN|VAL-","-TXT|RES-"], newton_window)
                hide(["-BTN|FILL-","-TXT|VX-", "-IN|VX-","-TXT|VAL-","-IN|VAL-","-TXT|EVAL-","YES","NO","-YES-","-NO-","-CANVAS-"], newton_window)
                reset_radio(["YES","NO","INTERVALO", "VALORES"], newton_window)
                try:
                    clear_canvas(canvas,newton_window,"-CANVAS-")
                except:
                    pass
                show(['-LBL|FUNCION-', '-TXT|FUNCION-',"-TXT|LIMIT-","INTERVALO","VALORES","-INTERVALO-","-VALORES-"], newton_window)
                newton_window['-TXT|FUNCION-'].update(showCalculator())
            ...
            #Se muestran los campos necesarios para la opción seleccionada "Tabla de datos"
                ...
                try:
                    ...
                    pass
                reset(["-IN|VX-", "-IN|GRD-","-IN|VAL-","-IN|LIMITI-","-IN|LIMITS-","-TXT|RES-","-TXT|FUNCION-"], newton_window)
                hide(["-BTN|FILL-","-CANVAS-","-IN|VX-", "-IN|GRD-","-IN|VAL-","-IN|LIMITI-","-IN|LIMITS-","-TXT|RES-","-TXT|FUNCION-","YES","NO","INTERVALO", "VALORES","-LBL|FUNCION-","-TXT|LIMIT-","-INTERVALO-","-VALORES-","-YES-","-NO-","-TXT|LIMITI-","-TXT|LIMITS-","-TXT|GRD-","-TXT|VX-","-TXT|EVAL-", "-TXT|VAL-","-TXT|RES-"], newton_window)
                show(["-TXT|VX-", "-IN|VX-","-BTN|FILL-"], newton_window)
        ...
        if event == "-BTN|FILL-":
            ...
            show(["-TXT|EVAL-","YES","NO","-YES-","-NO-"], newton_window)
        

        #Si el usuario dice que si, se le muestra el input para ingresar el valor a evaluar
        if event == "YES":
            show(["-TXT|VAL-","-IN|VAL-"], newton_window)
            hide(["-BTN|SOLVE-"], newton_window)
            Buttom_flag = False

        #Si el usuario dice que no, se oculta el input para ingresar el valor a evaluar
        elif event == "NO":
            hide(["-TXT|VAL-","-IN|VAL-"], newton_window)
            show(["-BTN|SOLVE-"], newton_window)
            Buttom_flag = True
        ...
        if event == "INTERVALO":  
            ...
            intervalo = True
            valores = False
            reset(["-IN|VX-","-IN|VAL-","-TXT|RES-"], newton_window)
            hide(["-TXT|VX-", "-IN|VX-","-BTN|FILL-","-CANVAS-"], newton_window)
            try:
                clear_canvas(canvas,newton_window,"-CANVAS-")
            except:
                pass
            uncheck("YES",newton_window)
            uncheck("NO",newton_window)
            show(["-TXT|LIMITI-","-IN|LIMITI-","-TXT|LIMITS-","-IN|LIMITS-","-TXT|GRD-","-IN|GRD-","-TXT|EVAL-","YES","NO","-YES-","-NO-"], newton_window)
            ...
            valores = True
            intervalo = False
            reset(["-IN|LIMITI-","-IN|LIMITS-","-IN|GRD-","-IN|VAL-","-TXT|RES-"], newton_window)
            hide(["-CANVAS-","-TXT|LIMITI-","-TXT|LIMITS-","-IN|LIMITI-","-IN|LIMITS-","-TXT|GRD-","-IN|GRD-","-TXT|EVAL-","YES","NO","-YES-","-NO-","-TXT|VAL-","-IN|VAL-"], newton_window)
            try:
                clear_canvas(canvas,newton_window,"-CANVAS-")
            except:
                pass
            uncheck("YES",newton_window)
            uncheck("NO",newton_window)
            show(["-TXT|VX-", "-IN|VX-"], newton_window)
        ...
        if event == "-BTN|CLR-":
            reset(["-IN|VX-", "-IN|GRD-","-IN|VAL-","-IN|LIMITI-","-IN|LIMITS-",'-COMBO|DATOS-',"-TXT|RES-","-TXT|FUNCION-"], newton_window)
            hide(["-BTN|FILL-","-CANVAS-","-IN|VX-", "-IN|GRD-","-IN|VAL-","-IN|LIMITI-","-IN|LIMITS-","-TXT|RES-","-TXT|FUNCION-","YES","NO","INTERVALO", "VALORES","-LBL|FUNCION-","-TXT|LIMIT-","-INTERVALO-","-VALORES-","-YES-","-NO-","-TXT|LIMITI-","-TXT|LIMITS-","-TXT|GRD-","-TXT|VX-","-TXT|EVAL-", "-TXT|VAL-","-TXT|RES-"], newton_window)
            reset_radio(["YES","NO","INTERVALO", "VALORES"], newton_window)
            clear_canvas(canvas,newton_window,"-CANVAS-")
        ...
        if event == "-BTN|SOLVE-":
            ...
            if values["-COMBO|DATOS-"] == "Tabla de datos":
                ...
                if values["-IN|VAL-"] != "":
                    respuesta, xi, fi, polinomio, result_eval = newton(datos=datos_usuario, valor= float(values["-IN|VAL-"]))
                    if grafico:
                        canvas = generate_grafic(xi=xi,fi=fi, metodo="Polinomio de Newton", window= newton_window, valor_eval=float(values["-IN|VAL-"]), result= result_eval, key="-CANVAS-",canvas=canvas)
                    else:
                        canvas = generate_grafic(xi=xi,fi=fi, metodo="Polinomio de  Newton", window= newton_window, valor_eval=float(values["-IN|VAL-"]), result= result_eval, key="-CANVAS-")
                    update(["-TXT|RES-"], newton_window, [respuesta])

                #Si no lo hay solo se pasan los datos ingresados por el usuario
                else:
                    respuesta, xi, fi, polinomio = newton(datos=datos_usuario)
                    if grafico:
                        canvas = generate_grafic(xi= xi,fi= fi, metodo="Polinomio de  Newton", window= newton_window, key="-CANVAS-", canvas=canvas)
                    else:
                        canvas = generate_grafic(xi= xi,fi= fi, metodo="Polinomio de  Newton", window= newton_window, key="-CANVAS-")

                    update(["-TXT|RES-"], newton_window, [respuesta])
                    show(["-TXT|RES-"], newton_window)
            ...
            #Si el ingreso de datos es atraves de una función matemática
                ...
                if intervalo:
                    ...
                    if values["-IN|VAL-"] != "":
                        respuesta, xi, fi, polinomio, result_eval, error_porcentual, error_teorico = newton(expresion=newton_window['-TXT|FUNCION-'].get(),  valor_eval= float(values["-IN|VAL-"]), intervalo=[values["-IN|LIMITI-"],values["-IN|LIMITS-"]], grado_polinomio=int(values["-IN|GRD-"]))
                        if grafico:
                            canvas = generate_grafic(xi=xi,fi=fi, metodo="Polinomio de  Newton", window= newton_window, valor_eval=float(values["-IN|VAL-"]), result= result_eval, key="-CANVAS-", canvas= canvas)
                        else:
                            canvas = generate_grafic(xi=xi,fi=fi, metodo="Polinomio de  Newton", window= newton_window, valor_eval=float(values["-IN|VAL-"]), result= result_eval, key="-CANVAS-")
                        update(["-TXT|RES-"], newton_window, [respuesta+error_porcentual+error_teorico])
                    
                    else:
                        respuesta, xi, fi, polisimple = newton(expresion=newton_window['-TXT|FUNCION-'].get(),  intervalo=[values["-IN|LIMITI-"],values["-IN|LIMITS-"]], grado_polinomio=int(values["-IN|GRD-"]))
                        if grafico:
                            canvas = generate_grafic(xi=xi,fi=fi, metodo="Polinomio de  Newton", window= newton_window, key="-CANVAS-", canvas=canvas)
                        else:
                            canvas = generate_grafic(xi=xi,fi=fi, metodo="Polinomio de  Newton", window= newton_window, key="-CANVAS-")
                ...
                #Si no lo hay solo se pasan los datos ingresados por el usuario
                    ...
                    if values["-IN|VAL-"] != "":
                        respuesta, xi, fi, polisimple, result_eval, error_porcentual, error_teorico = newton(expresion=newton_window['-TXT|FUNCION-'].get(), valor_eval= float(values["-IN|VAL-"]), datos=datos_usuario)
                        if grafico:
                            canvas = generate_grafic(xi=xi,fi=fi, metodo="Polinomio de  Newton", window= newton_window, valor_eval=float(values["-IN|VAL-"]), result= result_eval, key="-CANVAS-", canvas= canvas)
                        else:
                             canvas = generate_grafic(xi=xi,fi=fi, metodo="Polinomio de  Newton", window= newton_window, valor_eval=float(values["-IN|VAL-"]), result= result_eval, key="-CANVAS-")
                        ...
                        respuesta, xi, fi, polisimple = newton(expresion=newton_window['-TXT|FUNCION-'].get(), datos=datos_usuario)
                        if grafico:
                            canvas = generate_grafic(xi=xi,fi=fi, metodo="Polinomio de  Newton", window= newton_window, key="-CANVAS-", canvas=canvas)
                        else:
                            canvas = generate_grafic(xi=xi,fi=fi, metodo="Polinomio de  Newton", window= newton_window, key="-CANVAS-")
                        update(["-TXT|RES-"], newton_window, [respuesta])
            show(["-TXT|RES-","-CANVAS-"], newton_window)

            #Como ya se graficó minimo 1 vez entonces si hay un gráfico para borrar
            grafico = True  

        else:
            hide(["-BTN|CLR-"], newton_window)
